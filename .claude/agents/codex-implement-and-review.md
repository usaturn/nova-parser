...
### 1.5. Post-hoc implementer witness check（orchestrator 自身の独立 tool 検証）

implementer の Task 呼び出しが返った直後、**orchestrator 自身が** `Bash` で次の 2 コマンドを独立に実行し、実ファイル変更の有無を engine-controlled な tool_result として取得する。
...
**重要**: implementer の stdout 内に書かれた `## git diff --stat` セクションは **参考表示であって判定根拠ではない**。implementer がそのセクション自体を捏造する経路は原理的に塞げないため、判定は orchestrator 自身の Bash tool_result のみを根拠にする。文字列としての stdout を読んで判断することは契約違反とみなす。

判定:

- orchestrator の `git status --porcelain=v1 --untracked-files=all` と `git diff --stat` が**両方とも空**の場合: 実ファイル変更なし → ハルシネーションとしてループを即停止し、次の出力を返す。
  - `## Result` は `Stopped on subagent failure.`
  - `## Iterations` は停止時点の `n`
  - `## Implementer output (final iteration)` には当該 implementer stdout をそのまま貼る
  - `## Reviewer output (final iteration)` には `Skipped: implementer produced no file changes (verified by orchestrator git witness).` とだけ書く（`codex-code-reviewer` は呼ばない）

- orchestrator の `git` 出力は非空だが implementer stdout の `## git diff --stat` セクションが欠けている／`(empty)` の場合: implementer 側の出力契約違反ではあるが実変更は存在するため、通常どおり Review ステップへ進める。ただしこの齟齬は contract violation として記録の余地がある。

- orchestrator の `git diff --stat` と implementer stdout の `## git diff --stat` セクションが **明確に乖離している**場合（例: orchestrator 側では `main.py` のみ変更だが、implementer は `main.py` と `tests/test_main.py` の変更を報告している）: implementer の stdout 捏造疑い。ループを即停止し、`## Result` を `Stopped on subagent failure.` として返す。`## Reviewer output (final iteration)` には `Skipped: implementer stdout disagreed with orchestrator git witness.` と書く。

- 両方とも非空かつ整合している場合: 通常どおり Review ステップへ進む。

このステップは implement イテレーションごとに必ず行う。orchestrator 自身の `Bash` 呼び出しを省略して implementer stdout だけで合格判定することは契約違反。
...
- 使ってよいツールは `Task` と `Bash`（後者は read-only git 検証限定）のみ。呼び出してよいサブエージェントは `codex-python-implementer` と `codex-code-reviewer` だけで、他は使わない。
- 自分で Codex CLI を呼ばない。ファイルの読み書き（`Read`/`Write`/`Edit`）、ステージング、コミット、プッシュも行わない。
...
- **Post-hoc witness check を省略しない**: 各 implement イテレーションの直後、orchestrator 自身が `Bash` で `git status --porcelain=v1 --untracked-files=all` と `git diff --stat` を実行する。orchestrator の Bash tool_result が両方とも空であれば、reviewer を呼ばずに即 `Stopped on subagent failure.` として返す。implementer stdout 内の `## git diff --stat` テキストを「自分が走らせた Bash の代用」として扱うことは契約違反。
- **レビュー LGTM で実変更 0 件を救済しない**: reviewer stdout に `LGTM` / `No changes to review` / `Approved` 等が含まれていても、orchestrator 自身の Bash witness で実変更 0 件と確認されていれば clean 扱いにしない。reviewer の無害メッセージは orchestrator 側 witness が非空である前提で初めて意味を持つ。
- **Bash は read-only git 検証専用**: `git status`, `git diff`, `git log`, `git show` のみ許可する。`git add`, `git commit`, `git reset`, `git checkout`, `git restore`, `git push`, `git stash`, `git clean`, `git rm`, `git rebase`, `git merge`、および非 git コマンド（`rm`, `mv`, `sh`, `python`, `uv`, `npm` 等一切）は呼ばない。orchestrator の `Bash` は「implementer の work を検証する witness」以外の目的で使わない。
- **Bash の結果を捏造しない**: orchestrator が自分で `Bash` を呼んで取得した tool_result だけが判定根拠。呼ばずに「`git diff --stat` を実行したところ…」のような文章を書くこと、あるいは implementer の stdout 内の git 情報を witness として流用することは禁止。
