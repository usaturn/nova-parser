...
2. **worktree への展開**:
   ...
   - `git fetch origin pull/<PR_NUMBER>/head:pr-review/<PR_NUMBER>` を実行し、`git worktree add ../pr-review-<PR_NUMBER> pr-review/<PR_NUMBER>` で PR ブランチを別ディレクトリに展開する。
   - 同名の worktree が既に存在する場合は再利用する。壊れている場合は `git worktree prune` を実行してから再作成する。
...
7. **後片付け**: レビュー完了後、`git worktree remove ../pr-review-<PR_NUMBER>` で worktree を削除する。
