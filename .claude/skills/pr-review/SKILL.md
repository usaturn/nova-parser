...
allowed-tools: Read, Grep, Glob, Bash(gh *), Bash(git *), Bash(date *), Bash(uv run --no-project python *pr_review_workspace.py *), Edit(/tmp/pr-review-*/review-backup/**), Write(/tmp/pr-review-*/review-backup/**), Agent
---
...
ヘルパースクリプトは `.agents/skills/pr-review/scripts/pr_review_workspace.py` の実体を相対シンボリックリンクで共有しています。Codex 版と同一の実装・同一の保存ロックを使うため、両ランタイムのレビューが同じリポジトリへ並行して保存できます。
...
- Claude Code ランタイムが提示する正確なモデル識別子を `MODEL_NAME` として記録し、`create --model-name "$MODEL_NAME"` に一度だけ渡す。モデル系列の説明、設定ファイル、既存レビューのファイル名から推測・短縮・一般化しない
- 正確なモデル識別子を取得できない場合は、代替名・ファイル名からの推測で続行せず停止してユーザに確認する
- draft と destination は `create` が返した値をそのまま使用する。`save` では model、draft、destination を再指定しない
...
- サブエージェントは `$WT` 上のコードと PR 差分（`gh pr diff` の結果）のみ調査する。`reviews/` 全体と `$PREVIOUS_REVIEW` の読解はメインエージェントだけが行う
- 委譲時は `reviews/` を調査範囲から明示的に除外する。既存 review 本文・その要約・そこから抽出した観点を委譲 prompt に含めない。他モデルのレビュー本文・その要約・そこから抽出した観点をサブエージェント経由で受け取らない
...
2. **worktree への展開**:
   ...
   - `HELPER="$MAIN_ROOT/.claude/skills/pr-review/scripts/pr_review_workspace.py"` を記録する。以降、メインリポジトリに対する Git コマンドには必ず `git -C "$MAIN_ROOT"` を使い、カレントディレクトリへ暗黙に作用させない。
   - Claude Code ランタイムが提示する正確なモデル識別子を `MODEL_NAME` に設定する（`/` を含む provider 形式もそのまま渡してよい。ヘルパーがファイル名用に `/` を `-` へ置換する）。取得できない場合は既存 filename や系列名から推測せず停止してユーザに確認する。
   ...
   - dedicated ref fetch 後、次で GitHub の head と `PR_HEAD` の一致を確認する（HEAD consistency check 1/3）。不一致、空値、`gh` failure の場合は履歴を追記せず保存しない。
     ...
     ```
   - ランタイムから取得済みの正確な `MODEL_NAME` を使って `create` を一度だけ実行する。ヘルパーは `/tmp/pr-review-<PR_NUMBER>-<ランダム値>/worktree` に新しい detached worktree を作り（`--temp-root /tmp` で固定。`allowed-tools` の Edit/Write 許可と一致させる）、保存先 `$DESTINATION` の create 時フィンガープリント（存在有無、通常ファイル・directory・symlink・その他の種別、通常ファイルの内容 SHA-256 または symlink target text の SHA-256）を `session.json` に保存する。既存パスは再利用しない。destination が通常ファイルの場合はセッション backup へ snapshot し、そのパスを `previous_review` として返す。destination が directory・symlink・特殊 entry の場合は fail-closed で停止する。
     ```bash
     SESSION_JSON=$(uv run --no-project python "$HELPER" create --repo "$MAIN_ROOT" --pr-number "$PR_NUMBER" --commit "$PR_HEAD" --head-branch "$HEAD_BRANCH" --model-name "$MODEL_NAME" --temp-root /tmp)
     ```
   - `--model-name` を省略した場合、helper は worktree 作成前に停止する。モデル識別子に含まれる `/` はヘルパーが `-` に置換してファイル名に使う。空白・`..` など置換後も安全なファイル名にならない識別子は fail-closed で拒否する。
...
3. **既存レビューの読解（更新時）**:
   - `$PREVIOUS_REVIEW` が空でない場合だけ、その snapshot ファイルを読む。ライブ `$DESTINATION`、他モデルのレビュー本文、ディレクトリ一覧から自モデルを推測して読むことはしない。この読解はメインエージェントのみが行い、サブエージェントに委譲しない。既存 review 本文を委譲 prompt に含めない。
...
5. **コンテキスト読解（並列化）**:
   ...
   - 変更箇所の呼び出し元・呼び出し先の調査、依存関係ファイル（pyproject.toml / package.json / lock ファイル等）の確認、変更に対応する既存テストの探索は、Explore サブエージェントに並列で委譲してよい。委譲先の調査範囲は `$WT` 上のコードと PR 差分のみとする。`reviews/` と既存 review 本文は委譲範囲・prompt から除外する。
6. **プロジェクト固有観点の取り込み**: CLAUDE.md とプロジェクト内の規約・設計ドキュメントがあれば読み、そのプロジェクトの重点観点（例: XSS 多層防御、並列・増分ビルド互換性）を優先順位のチェックリストに加える。メインリポジトリ側と `$WT` 側の `reviews/` にある他モデルのレビューは情報源にしない（サブエージェントに委譲する場合も調査範囲から除外する）。
7. **候補洗い出し → 検証 → 報告**:
   - 優先順位に沿って候補指摘を洗い出し、検証プロトコルを通過したものだけを指摘フォーマットで報告する。
   - 更新時は既存 active 指摘を現在の差分と `$WT` のコードで再検証し、継続・解消・新規に分類する。再検証の最終判定と `$PREVIOUS_REVIEW` の解釈はメインエージェントが行う。
   - 継続指摘は ID・タイトル・検出時 HEAD を維持し、位置と根拠を現在値へ更新する。
...
反証試行で追加のコード追跡が必要な場合は、サブエージェントに調査を委譲してよい（調査範囲は `$WT` 上のコードと PR 差分のみ。`reviews/` と既存 review 本文は含めない）。ただし確度ラベルの最終判定は、メインコンテキストが根拠（コードパス・実行結果）を直接確認した上で行う。サブエージェントの報告のみを根拠に【検証済み】を付けない。
...
- 自モデルの `$PREVIOUS_REVIEW` 以外（ライブ `$DESTINATION`、他モデルのレビュー本文）を読んでいないか。既存 review 本文をサブエージェントへ渡していないか
...
- メインリポジトリ側と `$WT` 側の `reviews/<変換後ブランチ名>/` にある他モデルのレビュー本文を、自分でもサブエージェント経由でも開いたり検索対象に含めたりしていないか
...
- 正確な `$MODEL_NAME` を `--model-name "$MODEL_NAME"` で渡し、filename から推測していないか
