...
2. **worktree への展開**:
   ...
   - まず `git worktree list` で `pr-review-<PR_NUMBER>` を含む worktree が既に存在するか確認する（`git worktree list` の出力にはフルパスが表示されるため、`pr-review-<PR_NUMBER>` の部分一致で判定できる）。
   - 存在しない場合（新規展開）: `git fetch origin pull/<PR_NUMBER>/head:pr-review/<PR_NUMBER>` を実行し、`git worktree add .worktrees/pr-review-<PR_NUMBER> pr-review/<PR_NUMBER>` で PR ブランチを別ディレクトリに展開する。`.worktrees/` はリポジトリ直下の gitignore 済みディレクトリであり、親ディレクトリが書き込み不可な環境（devcontainer 等）でも動作する。
   - 既に存在する場合（前回レビューが後片付け未完了で残っている等）: その worktree ディレクトリ内で `git fetch origin pull/<PR_NUMBER>/head` の後 `git reset --hard FETCH_HEAD` で最新化して再利用する。ブランチが worktree にチェックアウト中のため、外側からの `git fetch origin ...:pr-review/<PR_NUMBER>` は失敗するので使わない。
   - worktree の管理情報が壊れている場合は `git worktree remove --force .worktrees/pr-review-<PR_NUMBER>`（または `git worktree prune`）で除去してから、新規展開のフローをやり直す。
   - 以降のファイル読解・テスト実行は worktree 側のディレクトリを対象に行う。
3. **差分収集**:
...
7. **後片付け**: レビュー完了後、`git worktree remove .worktrees/pr-review-<PR_NUMBER>` で worktree を削除する。`pr-review/<PR_NUMBER>` ローカルブランチが不要であれば `git branch -D pr-review/<PR_NUMBER>` も実行する。
