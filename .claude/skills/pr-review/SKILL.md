...
2. **worktree への展開**:
   ...
   - まず `git worktree list` で `pr-review-<PR_NUMBER>` の worktree が既に存在するか確認する。
   - 存在しない場合（新規展開）: `git fetch origin pull/<PR_NUMBER>/head:pr-review/<PR_NUMBER>` を実行し、`git worktree add ../pr-review-<PR_NUMBER> pr-review/<PR_NUMBER>` で PR ブランチを別ディレクトリに展開する。
   ...
   - worktree の管理情報が壊れている場合は `git worktree remove --force ../pr-review-<PR_NUMBER>`（または `git worktree prune`）で除去してから、新規展開のフローをやり直す。
...
7. **後片付け**: レビュー完了後、`git worktree remove ../pr-review-<PR_NUMBER>` で worktree を削除する。`pr-review/<PR_NUMBER>` ローカルブランチが不要であれば `git branch -D pr-review/<PR_NUMBER>` も実行する。
