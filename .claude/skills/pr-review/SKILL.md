...
2. **worktree への展開**:
   ...
   - 既に存在する場合（前回レビューが後片付け未完了で残っている等）: `$WT` 内で `git fetch origin pull/<PR_NUMBER>/head` の後 `git checkout --detach FETCH_HEAD` で明示的に detached HEAD として最新 PR head に揃えて再利用する。旧版手順が残した branch-attached worktree でも、`reset --hard` のようにブランチ tip を動かさず、既存ローカルブランチを保護する。
