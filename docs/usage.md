...
```
nova-parser [--mode {plain,structured,structured_tsv,gamedata,schema,docai,docai_plain,schema_propose,extract,crop}] [--schema SCHEMA] [--parallel-files PARALLEL_FILES] [--output-dir OUTPUT_DIR] [--schema-output SCHEMA_OUTPUT] [--min-card-area MIN] [--max-card-area MAX] [--padding PX] [files ...]
```
...
| `--schema-output` | `schema_propose` モードの出力先ファイルパス。指定時は親ディレクトリを自動作成 | — |
...
- `--output-dir` を指定した場合、出力ファイル、`extract` キャッシュ、Gemini JSON エラー調査用ファイルは指定先に保存されます。`schema_propose` では TSV 入力を省略した場合の走査元として使われ、出力先は `--schema-output` または `schema/` 配下で決まります
...
この章の `Output/` は `schema_propose` 以外の未指定時の既定出力先です。`--output-dir` を指定した場合は、指定先ディレクトリに読み替えてください。`schema_propose` の出力先は `--schema-output` または `schema/` 配下です。
...
docai TSV ファイル（`*.docai*.tsv`）を走査し、各 `## 型名` セクションのヘッダ行を解析してスキーマ提案 JSON を自動生成します。Gemini は使用しません。

| 項目 | 内容 |
|------|------|
| 入力 | 引数省略時は `Output/*.docai*.tsv`。ファイルやディレクトリを明示した場合は指定 TSV を処理 |
| 出力先 | `schema/` ディレクトリ、または `--schema-output` の指定先 |
| ファイル名 | 引数省略・単一ファイル・複数引数では `schema_proposal.json`。単一ディレクトリ入力では `schema_[ディレクトリ名].json` |
| エンコーディング | UTF-8 |
| フォーマット | JSON（インデント付き） |
...
uv run nova-parser --mode schema_propose
# → schema/schema_proposal.json

# 特定のファイルのみ指定
uv run nova-parser --mode schema_propose Output/TNX_OFC_020.docai.tsv Output/TNX_OFC_037.docai.tsv
# → schema/schema_proposal.json

# 単一ディレクトリ内の docai TSV を走査
uv run nova-parser --mode schema_propose Output/DX3_EA
# → schema/schema_DX3_EA.json
...
TSV ファイルやディレクトリを明示した場合、`schema_propose` はデフォルトの `Output/` を検証・作成しません。作業ディレクトリに `Output` という通常ファイルがある場合でも、指定 TSV を読み込んで `schema/` または `--schema-output` に書き込めます。TSV 入力を省略した場合だけ、`--output-dir`（未指定時は `Output/`）の `*.docai*.tsv` を走査します。
