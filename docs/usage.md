...
4. **型別 TSV 再生成**: 全画像分のキャッシュを集約し、スキーマ上の全型 `{type_name}.tsv` と当 run で出現した `none_{type_name}.tsv` を原子的に書き直す
...
TSV ファイルはキャッシュを元に毎回丸ごと書き直します（追記ではありません）。

- **スキーマに定義された全型** の `{type_name}.tsv` は run ごとに再生成されます。0 件の型でもヘッダ行のみのファイルが作られます
- **当 run で出現した unmatched 型** の `none_{type_name}.tsv` は毎回上書きされます
