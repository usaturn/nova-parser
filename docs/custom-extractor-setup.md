...
最小構成では `game_card` を共通 parent entity とし、画像中の文字列に直接対応する項目だけを child entity として定義します。`type_name` は既定では child field に含めません。カード種別が後段で必要なら、`skill_card` や `armor_card` のように parent label 自体を分けて `entity.type_` から復元する方が自然です。

> ラベル名の命名規則:
> Document AI の label name は先頭が英字で、使用できるのは英数字、`_`、`-` のみです。このガイドでは `game_card`, `name`, `ruby`, `skill_card` などの ASCII ラベル名を使います。日本語の「名称」「ルビ」「スキル」は label name ではなく、項目の意味を説明するための表現です。
...
> `type_name` の扱い:
> `type_name` は「画像内の文字列を抽出する field」ではなく分類値になりやすいため、Plain Text child として必須化しない方が安全です。カード面に種別名の文字列が明示されていて、その文字列自体を bounding box でラベル付けできる場合だけ、例外的に追加を検討してください。

### 「This is a parent label」とは

「新しいラベルを作成」ダイアログの **This is a parent label** は、その label を値そのものではなく、複数の子 label を束ねる container として扱う指定です。

- チェックあり: 親 label。子 label をぶら下げて 1 レコード単位にまとめる
- チェックなし: 通常の field label。`name` や `ruby` のように実際の文字列へ紐づける
- このガイドでは `game_card` が親 label に該当する。`name` や `description` を同じカードに属する子 label としてまとめるために使う

親 label は record 単位で繰り返される前提なので、Occurrence は `Optional Multiple` または `Required Multiple` を選ぶのが前提です。

### field description の書き方

field description は抽出精度に影響します。TRPG カードでは類似フィールドが多いため、各フィールドが何を指すか明確に記述します。`Output/schema.json` には `名称` や `防(S/P/I)` のような raw field 名が含まれますが、Custom Extractor の label name にはそのまま使わず、ASCII の正規化ラベルへ寄せて定義します。

カード種別ごとに必要な label だけを作成してください。以下は `Output/schema.json` に出現した field を、Document AI 用の推奨 label name に写像した例です。

| 推奨 label name | `schema.json` 上の元フィールド | 用途・対象カード | description 例 |
|-----------------|-------------------------------|------------------|----------------|
| `name` | `名称` | 全カード共通 | データ項目の固有名称。カード内で最も大きく表示される文字列 |
| `ruby` | `ルビ` | 全カード共通 | `name` のふりがな。`name` の直上または直下に小さい文字で記載される |
| `description` | `解説` | 全カード共通 | カードの解説テキスト。カード内で最も長い文章部分 |
| `prerequisite` | `技能` | スキルカード | スキルの前提技能または使用技能を示す欄。「技能」と見出されることが多い |
| `max_level` | `上限` | スキルカード | スキルの上限レベル。数値で記載されることが多い。装備の「制」とは別項目 |
| `timing` | `タイミング` | スキルカード | 発動タイミング。メジャー、マイナー、常時などの語が入る |
| `target` | `対象` | スキルカード | 効果対象。単体、自身、範囲などの語が入る |
| `range` | `射程`, `射程: `, `射` | スキルカード、武器、ヴィークル | 射程を表す欄。`射程`、`射程: `、`射` の表記ゆれは同じ label に寄せる |
| `target_value` | `目標値` | スキルカード | 判定に使う目標値の欄。数値や難易度表記が入る |
| `opposed` | `対決` | スキルカード | 対決の有無や対決技能を示す欄 |
| `manufacturer` | `メーカー` | 装備系カード | 製造元、ブランド名、企業名などを示す欄 |
| `purchase` | `購` | 装備系カード | 購入価格。「購」と略記される。防御力や攻撃力ではない |
| `concealment` | `隠` | 装備系カード | 隠匿値。「隠」と略記される |
| `attack` | `攻` | 武器、ヴィークル | 攻撃力を示す欄。装備制限や購入価格ではない |
| `ju` | `受` | 武器 | 原表記 `受` の値。意味を推測し過ぎず、カード上の値をそのまま抽出する |
| `su` | `ス` | 武器、IANUS、ポケットロン、ヴィークル | 原表記 `ス` の値。略記 field のため raw value をそのまま抽出する |
| `defense_spi` | `防(S/P/I)` | 防具、ヴィークル | 防御値をまとめて表す欄。`S/P/I` の3値を含む表記を1つの field として扱う |
| `restriction` | `制` | 装備系カード | 装備や使用の制限値。「制」と略記される。スキルの `上限` とは別項目 |
| `electric_restriction` | `電制` | 装備系カード | 電制値。「電制」と略記される |
| `slot` | `部位` | 装備系カード | 装備部位。片手持ち、スーツ、胴部などが入る |
| `hyo` | `表` | IANUS | 原表記 `表` の値。意味を独自解釈せず raw value を保持する |
| `shin` | `深` | IANUS | 原表記 `深` の値。意味を独自解釈せず raw value を保持する |
| `mu` | `無` | IANUS | 原表記 `無` の値。意味を独自解釈せず raw value を保持する |
| `sa` | `サ` | タップ | 原表記 `サ` の値。略記 field のため raw value をそのまま抽出する |
| `so` | `ソ` | タップ | 原表記 `ソ` の値。略記 field のため raw value をそのまま抽出する |
| `ha` | `ハ` | タップ | 原表記 `ハ` の値。略記 field のため raw value をそのまま抽出する |
| `cs` | `CS` | タップ | 原表記 `CS` の値。略記 field のため raw value をそのまま抽出する |
| `sf` | `SF` | ヴィークル | 原表記 `SF` の値。略記 field のため raw value をそのまま抽出する |
| `crew` | `乗員` | ヴィークル | 乗員数を示す欄。人数や定員の表記が入る |

特に `防(S/P/I)` のような不正な raw field 名や、`射程: ` のような表記ゆれはそのまま label にしないでください。意味が未確定な略記 (`受`, `ス`, `表`, `深`, `無`, `サ`, `ソ`, `ハ`, `CS`, `SF`) は、まず source-close な ASCII label name を割り当て、description に元表記を明記して運用するのが安全です。
...
4. `game_card` の下に「Add child field」で子フィールドを追加
   - 各フィールドの name は英字始まりの ASCII ラベル名で設定する
   - 各フィールドの data type, occurrence は上の表に従って設定
...
2. 文書が表示されたら、以下の手順でラベル付けを行う:
   1. まず子ラベル（`name`, `ruby` 等）の bounding box を実際の文字列に対して配置する
   2. 同じカードに属する子ラベル同士を、同一の `game_card` 親エンティティに紐づける
   3. 各子ラベルのテキストが正しく選択され、親子関係が正しいことを確認する
3. すべてのカードとフィールドをラベル付けしたら「Mark as labeled」を選択
4. 次の文書に移り、すべての文書でラベル付けを完了する

### parent-child ラベル付けのポイント

- **親は container**: `game_card` は値を直接抜くための label ではなく、同じカードに属する子 label を束ねるための親エンティティ
...
- [ ] すべてのカードの子ラベルが、いずれかの `game_card` 親エンティティに紐づいている
- [ ] 同じカード内の子ラベルが、同一の `game_card` にまとまっている
...
TYPE_NAME_BY_ENTITY = {
    "skill_card": "スキル",
    "armor_card": "防具",
    "melee_weapon_card": "白兵武器",
}

def extract_game_cards(document: documentai.Document) -> list[dict]:
    """ASCII の parent label を repo 互換の type_name に写像する"""
    cards = []

    for entity in document.entities:
        type_name = TYPE_NAME_BY_ENTITY.get(entity.type_)
        if type_name is None:
            continue

        card = {"type_name": type_name}
