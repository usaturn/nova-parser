...
2. **矩形作成**: 画像上をマウスでドラッグ。閾値（5px）未満のドラッグは破棄。通常モード（ブロック選択 OFF）では、ズームツールバーの「手描き読み順」セレクタで新規矩形の `reading_order` を選べる（初期値は「横書き（既定）」）
...
### `Rectangle.reading_order`

`{stem}.regions.json` の各矩形は `reading_order` を持ちます。
...
ツールバーの「手描き読み順」で選んだ値は、選択した後に新しく手描きする矩形にのみ適用されます。既に作成済みの矩形の読み順を変えたい場合は、その矩形を削除してからセレクタで選び直し、再度ドラッグして描き直してください（`reading_order` は作成後に変更できません）。

フィールドが無い旧形式のセッション JSON は `vision` として読み込まれるため、既存ファイルの挙動は変わりません。既に作成済みの矩形へ縦書きの読み順を適用したい場合は、その矩形を削除し、縦ブロック粒度で選択し直してから OCR してください（`reading_order` は作成後に変更できません）。
