...
| 値 | 付与される条件 | OCR 時の挙動 |
|---|---|---|
| `vision` | 既定。ドラッグ作成・横ブロック・段落ブロック | Cloud Vision の `full_text_annotation.text` をそのまま保存する |
| `vertical` | 縦ブロック粒度でブロックをクリックして作成 | block 座標から列は右→左、列内は上→下に並べ替えてテキストを組み立てる。複数列を横断する幅広 block（帯見出しなど）はいずれの列にも吸収せず、Y 位置に応じて列群の前後へ挟み込む。構造化 block が返らない場合は `vision` と同じテキストへフォールバックする |
